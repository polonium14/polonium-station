...
public sealed partial class ForensicsSystem : EntitySystem
{
    ...
    private void OnUtilityVerb(Entity<CleansForensicsComponent> entity, ref GetVerbsEvent<UtilityVerb> args)
    {
        ...
        // Only offer the verb for blood cult runes or entities that carry forensics.
        var isRune = HasComp<CleanableRuneComponent>(args.Target);
        var hasForensics = _forensicsQuery.HasComp(args.Target);
    ...
    {
        // Blood cult runes can be scrubbed away with soap.
        if (HasComp<CleanableRuneComponent>(target))
        {
            var runeDelay = cleanForensicsEntity.Comp.CleanDelay;
            var runeDoAfter = new DoAfterArgs(EntityManager, user, runeDelay, new CleanForensicsDoAfterEvent(), target, target: target, used: cleanForensicsEntity)
            {
                NeedHand = true,
                BreakOnDamage = true,
                BreakOnMove = true,
            ...
            if (!_doAfterSystem.TryStartDoAfter(runeDoAfter))
                return false;
    ...
    [SubscribeLocalEvent]
    private void OnCleanRuneDoAfter(Entity<CleanableRuneComponent> ent, ref CleanForensicsDoAfterEvent args)
    {
        if (args.Handled || args.Cancelled || args.Args.Target == null)
            return;
