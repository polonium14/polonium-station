...
public sealed partial class FlashImmunitySystem : EntitySystem
{
    ...
    {
        uid = GetPossibleWearer(uid);
        FlashImmunityCheckEvent flashImmunityChangedEvent =
            new(uid, HasFlashImmunityVisionBlockers(uid));

        RaiseLocalEvent(uid, flashImmunityChangedEvent);
    }

    private void OnThermalVisionToggled(
        EntityUid uid,
        ThermalVisionComponent component,
        ref SwitchableOverlayToggledEvent args)
    {
        uid = GetPossibleWearer(uid);

        var flashImmunityChangedEvent =
            new FlashImmunityCheckEvent(uid, HasFlashImmunityVisionBlockers(uid));
