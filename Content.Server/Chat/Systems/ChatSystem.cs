...
public sealed partial class ChatSystem : SharedChatSystem
{
        ...
        )
    {
        ...
        if (playSound)
        {
            ...
            if (sound == null)
            {
                sound = sender == Loc.GetString("admin-announce-announcer-default")
                    ? CCAnnouncementSound
        ...
        Color? colorOverride = null)
    {
        ...
        if (playDefaultSound)
        {
            _audio.PlayGlobal(announcementSound ?? DefaultAnnouncementSound, filter, true, AudioParams.Default.WithVolume(-3f));
        }
