    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (mix.GetMoles(Gas.AntiNoblium) > 0.01f && mix.GetMoles(Gas.Helium) > 0.01f)
        {
            ...
            if (consumedAN > 0)
            {
                ...
                if (sm.ZapLast + TimeSpan.FromSeconds(5) <= _timing.CurTime)
                {
                    _lightning.ShootRandomLightnings(uid, zapRange, zapCount, sm.LightningPrototypes[zapPower]); sm.ZapLast = _timing.CurTime;
                }
            }
        }
