...
public sealed partial class GasFilterSystem : SharedGasFilterSystem
{
    ...
    private void OnFilterUpdated(Entity<GasFilterComponent> ent, ref AtmosDeviceUpdateEvent args)
    {
        ...
        HashSet<Gas>? gasesToFilter = null;
        if (ent.Comp.FilterGases.Count > 0)
            gasesToFilter = ent.Comp.FilterGases;
        else if (ent.Comp.FilteredGas is { } filtered)
            gasesToFilter = new HashSet<Gas> { filtered };

        if (gasesToFilter != null)
        {
            ...
            var remainingLimit = limitMolesFilter;
            var totalFiltered = 0f;

            foreach (var gas in gasesToFilter)
            {
                if (remainingLimit <= 0f)
                    break;

                var availableMoles = removed.GetMoles(gas);
                var filteredMoles = Math.Max(Math.Min(remainingLimit, availableMoles), 0f);
                if (filteredMoles <= 0f)
                    continue;

                filteredGasMixture.SetMoles(gas, filteredMoles);
                removed.AdjustMoles(gas, -filteredMoles);
                remainingLimit -= filteredMoles;
                totalFiltered += filteredMoles;
            }

            _atmosphereSystem.Merge(filterNode.Air, filteredGasMixture);
            _ambientSoundSystem.SetAmbience(ent.Owner, totalFiltered > 0f);
        }
