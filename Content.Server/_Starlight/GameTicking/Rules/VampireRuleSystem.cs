...
public sealed partial class VampireRuleSystem : GameRuleSystem<VampireRuleComponent>
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<VampireRuleComponent, AfterAntagEntitySelectedEvent>(AfterEntitySelected);
        SubscribeLocalEvent<VampireRuleComponent, ObjectivesTextPrependEvent>(OnTextPrepend);
    }

    private void AfterEntitySelected(EntityUid uid, VampireRuleComponent comp, ref AfterAntagEntitySelectedEvent args)
    ...
    public bool MakeVampire(EntityUid target, VampireRuleComponent rule)
    {
        ...
        if (vampRole is not null)
        {
            EnsureComp<RoleBriefingComponent>(vampRole.Value.Owner, out var briefingComp);
