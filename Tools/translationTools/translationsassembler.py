...
class TranslationsAssembler:
    ...
    def execute(self):
        for keys in self.group:
            ...
            pl_full_path = self.group[keys][0].get_file_path().pl
            pl_file = FluentFile(pl_full_path)
            try:
                ru_file_parsed = pl_file.read_parsed_data()
            except:
                logging.error(f'Plik {pl_file.full_path} nie może zostać sparsowany. Pomiń go.')
                continue
            ...
            if len(for_update):
                updated_pl_file_parsed = manager.update(for_update)
                updated_pl_file_serialized = FluentSerializer(with_junk=True).serialize(updated_pl_file_parsed)
                pl_file.save_data(updated_pl_file_serialized)

                updated_keys = list(map(lambda el: el.get_id_name(), for_update))
                logging.info(f'Zaktualizowano klucze w pliku {pl_file.full_path}: {updated_keys}')
