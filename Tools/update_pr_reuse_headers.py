...
def process_git_log_output(output, author_timestamps, ignored_authors=None):
    ...
    for line in output.splitlines():
        ...
        # Add main author
        if author_name and author_email and author_name.strip() != "Unknown" and author_name.strip().lower() not in {"funkystationbot", "polonium-bot", "pjbot"}:
            should_ignore = False
            if ignored_authors:
                for ignored in ignored_authors:
        ...
        for match in co_author_regex.finditer(body):
            ...
            if co_author_name and co_author_email and co_author_name.strip() != "Unknown" and co_author_name.strip().lower() not in {"funkystationbot", "polonium-bot", "pjbot"}:
                should_ignore = False
                if ignored_authors:
                    for ignored in ignored_authors:
...
def create_header(authors, license_id, comment_style):
    ...
    if suffix is None:
        ...
        if authors:
            ...
            lines.append(f"{prefix} SPDX-FileCopyrightText: Contributors to the Polonium Station project")
        ...
        if authors:
            ...
            lines.append(f"SPDX-FileCopyrightText: Contributors to the Polonium Station project")
