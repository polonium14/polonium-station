
marking-ThavenEars1-ears1 = Small Ears
marking-ThavenEars1 = Small Ears
marking-ThavenEars2-ears2 = Medium Ears
marking-ThavenEars2 = Medium Ears
marking-ThavenEars3-ears3 = Long Ears
marking-ThavenEars3 = Long Ears
marking-ThavenEars4-ears4 = Droopy Ears
marking-ThavenEars4 = Droopy Ears
marking-ThavenPiercings-piercings = Hoop Piercing
marking-ThavenPiercings = Hoop Piercing
marking-ThavenPiercings2-piercings2 = Helix Piercing
marking-ThavenPiercings2 = Helix Piercing
marking-ThavenChestTattoo1-chesttat1 = Back Tattoo
marking-ThavenChestTattoo1 = Back Tattoo
marking-ThavenLArmTattoo1-larmtat1 = Arm Band (Left)
marking-ThavenLArmTattoo1 = Arm Band (Left)
marking-ThavenRArmTattoo1-larmtat1 = Opaska na ramię (Prawa)
marking-ThavenRArmTattoo1 = Arm Band (Right)
marking-ThavenRArmTattoo1-rarmtat1 = Arm Band (Right)
marking-ThavenREyeGlow-reyeglow = Glowing Eye (Right)
marking-ThavenREyeGlow = Glowing Eye (Right)
marking-ThavenLEyeGlow-leyeglow = Glowing Eye (Left)
marking-ThavenLEyeGlow = Glowing Eye (Left)
marking-ThavenTailEel-taileel1 = Tail
marking-ThavenTailEel-taileel2 = Fin
marking-ThavenTailEel = Eel Tail (Underlay)
marking-ThavenTailEelOverlay-taileel3 = Eel Tail (Overlay)
marking-ThavenTailEelOverlay = Eel Tail (Overlay)
marking-ThavenTailNurse-tailnurse1 = Tail
marking-ThavenTailNurse-tailnurse2 = Stripes
marking-ThavenTailNurse = Swept Tail
marking-ThavenTailStub-tailstub1 = Tail
marking-ThavenTailStub-tailstub2 = Stripes
marking-ThavenTailStub = Stub Tail
# the ftl file got lost and i genuinely cant find it, dunno if these are from DV or imp
marking-ThavenHeadCap = Head Cap
marking-ThavenHeadCap-thavenheadcap1 = Head Cap (Primary)
marking-ThavenHeadCap-thavenheadcap2 = Head Cap (Secondary)
marking-ThavenHeadStripes = Stripes
marking-ThavenHeadStripes-thavenheadstripes = Stripes
marking-SharkminnowEyeliner = Sharkminnow Eyeliner
marking-SharkminnowEyeliner-thaventattoosharkminnoweyeliner = Sharkminnow Eyeliner
marking-ThavenHeadScales = Head Scales
marking-ThavenHeadScales-thavenheadscales = Scales
marking-ThavenEarsBigFins = Big Fins
marking-ThavenEarsBigFins-thavenearsbigfins = Big Fins (Primary)
marking-ThavenEarsBigFins-thavenearsbigfins2 = Big Fins (Secondary)
marking-ThavenFishEars = Fish Ears
marking-ThavenBiteMark = Bite Mark
marking-ThavenBiteMark-thavenbitemark = Bite Mark
marking-ThavenBodyStripes = Body Stripes
marking-ThavenBodyStripes-thavenbodystripes = Body Stripes
marking-ThavenChestScales = Chest Scales
marking-ThavenChestScales-thavenchestscales = Chest Scales
marking-ThavenSpines = Spines
marking-ThavenSpines-thavenspines = Spines
marking-ThavenTattooVines = Tattoo Vines
marking-ThavenTattooVines-thaventattoovines = Tattoo Vines
marking-ThavenTattooWave = Tattoo Wave
marking-ThavenTattooWave-thaventattoowave = Tattoo Wave
marking-ThavenTiger = Tiger Underbelly
marking-ThavenTiger-thaventiger1 = Tiger Underbelly (Primary)
marking-ThavenTiger-thaventiger2 = Tiger Underbelly (Secondary)
marking-ThavenTopSurgery = Top Surgery
marking-Gills = Gills
marking-Gills-thavenchestgills = Gills
marking-ThavenLArmScales = Scales (Left)
marking-ThavenLArmScales-thavenleftarmscales = Scales (Left)
marking-ThavenRArmScales = Scales (Right)
marking-ThavenRArmScales-thavenrightarmscales = Scales (Right)
marking-ThavenTigerRArm = Tiger (Right)
marking-ThavenTigerRArm-thaventigerarmsr = Tiger (Right)
marking-ThavenTigerLArm = Tiger (Left)
marking-ThavenTigerLArm-thaventigerarmsl = Tiger (Left)
marking-ThavenLLegScales = Scales (Left)
marking-ThavenLLegScales-thavenleftlegscales = Scales (Left)
marking-ThavenRLegScales = Scales (Right)
marking-ThavenRLegScales-thavenrightlegscales = Scales (Right)
marking-ThavenArmFinsBigLeft = Large Fin (Left)
marking-ThavenArmFinsBigLeft-larmfins2 = Large Fin (Left)
marking-ThavenArmFinsBigRight = Large Fin (Right)
marking-ThavenArmFinsBigRight-rarmfins2 = Large Fin (Right)
marking-ThavenArmFinsSmallLeft = Small Fin (Left)
marking-ThavenArmFinsSmallLeft-larmfins1 = Small Fin (Left)
marking-ThavenArmFinsSmallRight = Small Fin (Right)
marking-ThavenArmFinsSmallRight-rarmfins1 = Small Fin (Right)
marking-ThavenLegFinsSmallLeft = Leg Fin (Left)
marking-ThavenLegFinsSmallLeft-rlegfins1 = Leg Fin (Left)
marking-ThavenLegFinsSmallRight = Leg Fin (Right)
marking-ThavenLegFinsSmallRight-llegfins1 = Leg Fin (Right)
marking-ThavenBackFin = Dorsal Fin
marking-ThavenBackFin-backfin = Dorsal Fin
marking-ThavenTailCarpish = Carpish Tail
marking-ThavenTailCarpish-carpish = Carpish Tail
marking-ThavenTailFish = Fishy Tail
marking-ThavenTailFish-fishtail = Fishy Tail
marking-ThavenTailDraconic = Draconic Tail
marking-ThavenTailDraconicLong = Draconic Tail
marking-ThavenTailDraconicLong-draconiclong = Draconic Tail
marking-ThavenHeadFinsFairy = Fairy Tail
marking-ThavenHeadFinsFairy-fairyfins = Fairy Tail
marking-ThavenHeadAnglerStem = Angler Stem
marking-ThavenHeadAnglerStem-angler-stem = Angler Stem
marking-ThavenHeadAnglerBulb = Angler Bulb
marking-ThavenHeadAnglerBulb-angler-bulb = Angler Bulb
