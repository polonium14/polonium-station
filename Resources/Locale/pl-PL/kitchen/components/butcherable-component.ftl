butcherable-different-tool = Bedziesz {GENDER($user) ->
        [male] potrzebował
        [female] potrzebowała
        *[other] potrzebowało
    } innego narzędzia aby rozłożyć { THE($target) }.
butcherable-knife-butchered-success = Rozłożyłeś { THE($target) } używając { THE($knife) }.
...
butcherable-mob-isnt-dead = Musi być {GENDER($target) ->
        [male] martwy
        [female] martwa
        *[other] martwe
    }.
