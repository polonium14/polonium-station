...
public sealed partial class LayerMarkingItem : BoxContainer, ISearchableControl
{
    ...
    {
        MarkingTexture.Textures = _markingPrototype.Sprites.Select(GetMarkingIcon).ToList();
        SelectButton.Text = Loc.GetString($"marking-{_markingPrototype.ID}");
    }

    private Texture GetMarkingIcon(SpriteSpecifier sprite)
    {
        if (_layer is not (HumanoidVisualLayers.Tail or HumanoidVisualLayers.TailOverlay))
            return _sprite.Frame0(sprite);

        return _sprite.RsiStateLike(sprite).TextureFor(Direction.North);
    }
