...
public sealed partial class SupermatterEntryContainer : BoxContainer
{
    ...
    public SupermatterEntryContainer(NetEntity uid)
    {
        ...
        foreach (var gas in gasData)
        {
            ...
            if (!_proto.HasIndex<GasPrototype>(gas.ToString()))
                continue;

            _gasBarData.Add((new SupermatterGasBarContainer(gas), gas));
        }
        ...
        _radiationBase = SupermatterConsoleCVars.GetFloat(_config, ECCVars.SupermatterRadsBase);
        _temperatureLimitBase = Atmospherics.T0C + SupermatterConsoleCVars.GetFloat(_config, ECCVars.SupermatterHeatPenaltyThreshold);
        _wasteBase = 1f;
