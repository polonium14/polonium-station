...
public sealed class TutorialHighlightOverlay : Control
{
    ...
    /// <summary>
    /// Placement only happens on the next frame, so keep the control out of sight until then -
    /// otherwise it renders one frame at the origin and flashes in the corner.
    /// </summary>
    private static void ParkOffScreen(Control overlayControl)
    {
        LayoutContainer.SetPosition(overlayControl, new Vector2(-100000f, -100000f));
    }
    ...
    /// <summary>True once the highlighted control is gone, e.g. the player switched hud layout.</summary>
    public bool IsHighlightStale => CurrentHighlight is { } h
                                    && (h.Control.Parent == null || !h.Control.VisibleInTree);
